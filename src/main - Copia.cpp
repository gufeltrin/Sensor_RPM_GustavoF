...
long rpm =0; //variavel utilizada para calcular o rpm
long rpm_resolucao =0; //rpm com resolução de 20 que será enviado para a CAN
...
void setup()
{
  ...
  Timer1.initialize(callback); // Inicializa o Timer1 e configura para um período de 0,04 segundos
...
void sensorRPM(void)
{
  ...
  long tempoChamadoUltimaVez; //marca o tempo anterior
...
void enviarCAN()
{
  if (tmrFuncOverflow == true) {
    canMsg.data[0] = rpm;               //envia o valor de rpm na posição [0]
